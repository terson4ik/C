/* Square and running dog: extended ++ extended version */
...
typedef struct {
    ...
    unsigned char cur_colr;
} runner;
#define FIRST_COLOR black
#define LAST_COLOR  white
...
int main(void)
{
    ...
    if (!has_colors()) {
        endwin();
        fputs("This terminal doesn't support colors. Use past version.\n", 
            stderr);
        return 1;
    }
    curs_set(0);
...
void move_runner(runner *r, const rectangle *sqr)
{
    ...
    r->cur_colr = (r->cur_colr + 1 > COUNT_COLORS) ? 1 : r->cur_colr + 1;
    
    attrset(COLOR_PAIR(r->cur_colr));
